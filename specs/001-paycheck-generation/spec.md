...
### Session 2025-12-23
- Q: How is sales commission data sourced? → A: Manager manually enters total sales amount for each employee for the period before generation.
- Q: How do public holidays affect attendance bonuses (Sunday payment/Lunch)? → A: Count as "Worked": Holidays trigger Sunday payment eligibility and do not include Lunch benefit.
- Q: How should the system handle scheduling if a skill requirement cannot be met without conflict (e.g., only one employee available)? → A: Soft Constraint (Warn): Generate the best possible schedule but flag the conflict (e.g., overlapping shifts, missing personnel) for manager review.
- Q: How is attendance data ingested from the fingerprint reader? → A: CSV/Excel Upload: Manager uploads a file exported from the reader.
- Q: How are "Offline Reader" scenarios handled? → A: Manual Override: Manager creates manual "Worked" records for affected employees/dates.
...
4. **Given** specific sales percentages for an employee, **When** I manually input their total sales and generate, **Then** the commission is added to the total.
...
- **Offline Reader**: If the reader fails, the Manager can manually create "Worked" records to ensure benefits are correctly calculated.
- **Public Holidays**: Holidays are treated as fully worked days, preserving Sunday payment eligibility and denying the Lunch benefit.
- **Skill Scarcity**: If a skill requirement (e.g., Storage) cannot be met without overlapping shifts due to lack of staff, the system provides a warning but generates a "best-effort" schedule.
...
- **FR-002.1**: System MUST provide an interface for the Manager to manually input total sales figures for each employee prior to paycheck generation for the period.
...
- **FR-005**: System MUST allow the Manager to upload attendance logs via CSV or Excel file (containing Employee ID and Timestamp) to simulate the fingerprint reader interface.
- **FR-006**: System MUST register a "Full Working Day" only if both morning and afternoon check-ins are present for a given date.
- **FR-007**: System MUST generate schedules ensuring employees with the same exclusive skillset (e.g., Storage) are not scheduled in the same time block (Morning vs Afternoon).
- **FR-007.1**: System MUST flag unavoidable skill-based scheduling conflicts for Manager review if staff availability is insufficient.
- **FR-008**: System MUST allow manual override of attendance records before paycheck generation.
- **FR-009**: System MUST treat designated Public Holidays as "Full Working Days" for calculation purposes (Lunch benefit awarded, Sunday payment streak preserved).

### Key Entities

- **Employee**: ID, Name, Skills (List), Role, Base Salary, Sales % config.
- **PaycheckConfig**: ID, Global rules for Health %, Retirement %, Lunch value, Sunday value.
